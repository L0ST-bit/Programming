...
public:
    ...
    Rational(int a, int b) : num(abs(a)), den(abs(b)), sign(whatSign(a) == whatSign(b) ? 1 : -1) {
        if (a != 0 and b != 0) {
    ...
    Rational operator +(const Rational& r) const {
        ...
        return Rational((sign * num * r.den) + (r.sign * r.num * den), den * r.den);
    }
    ...
    Rational operator /(const Rational& r) const {
        auto res = Rational(num * r.den, den * r.numerator());
